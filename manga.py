...
class MangaTranslator:
    ...
    def __init__(
        ...
        self._lama = None
        self._fast_lama = None
        self._lama_lock = threading.Lock()
    ...
    def _get_lama(self):
        ...
        if self._lama is not None or not (self.use_lama or getattr(self, "inpaint_auto", True)):
            return self._lama
        with self._lama_lock:
                ...
                try:
                    ...
                    for (cx0, cy0, cx1, cy1) in self._mask_clusters(dil):
                        ...
                        if need_quality:
                            ...
                            if getattr(self, "_fast_lama", None) is None:
                                with self._lama_lock:
                                    if getattr(self, "_fast_lama", None) is None:
                                        self._fast_lama = LamaONNX(prefer_gpu=self.use_gpu)
                            fast_inpainter = self._fast_lama
...
"""
    ...
    def run(self, input_path: str, output_path: str, resume: bool = True,
        ...
        if len(pending) <= 1:
            ...
            extract_w = max(1, min(int(self.max_workers), 2))
            post_w = max(1, min(int(self.max_workers), 2, len(pending)))
