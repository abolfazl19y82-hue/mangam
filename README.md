...
| قابلیت | توضیح |
|--------|--------|
| **برنامهٔ  (`manga_app.py`)** | یک فایل، سه حالت: **App** (پنجرهٔ دسکتاپ تیره با Tkinter)، **Web** (رابط Gradio تیره — خودکار روی Colab/Codespace بالا می‌آید و لینک عمومی می‌دهد)، **CLI تعاملی** (منوی فارسی می‌پرسد). لانچرهای `Manga.bat` و `manga.sh` منودار |
| **پاکسازی خودکار (پیش‌فرض)** | برای هر خوشهٔ متن بهترین روش خودکار انتخاب می‌شود: پس‌زمینهٔ تخت → پرکردن مستقیم (بی‌نقص و آنی)، ساده → LaMa سریع ۲۵۶، آرت/متن تیره یا ماسک بزرگ → **LaMa-Manga ۵۱۲** |
| **LaMa-Manga (fine-tune مانگا)** | اینپینتر پیش‌فرض پاکسازی — مدل LaMa آموزش‌دیده روی مانگا/انیمه، نسخهٔ ONNX و سبک؛ روی CPU هم اجرا می‌شود |
| **پاکسازی «فقط حروف»** | حباب هیچ‌وقت کامل پاک نمی‌شود؛ ماسک adaptive فقط حروف را می‌گیرد (روی glow/گرادیان هم کار می‌کند) + دور دومِ جاروی جوهر جامانده |
| **گارد فونت** | فونت‌های لحن بار اول خودکار دانلود می‌شوند (کودک، افسانه، کروش، زمزمه، تفکر، سیستم، نامه، راوی، متن آزاد) و **پوشش حروف فارسی هر فونت تست می‌شود**؛ فونت ناقص (مربع‌دار) خودکار کنار گذاشته می‌شود |
| **تشخیص تگ چرخیده** | آستانهٔ تشخیص پایین‌تر آمد (0.16) — تگ‌های چرخیده/نیمه‌شفاف مثل `AIDE ZEPHYROS LV.15` پیدا و با همان زاویه رندر می‌شوند |
| **واترمارک هوشمند** | تشخیص توکن‌محور — `ASURASCANS.COM` ترجمه/پاک نمی‌شود و retry بیهوده نمی‌خورد؛ اسم‌هایی مثل `ASURAT` اشتباهی واترمارک حساب نمی‌شوند |
| **ترجمهٔ پایدار** | چند حباب در هر درخواست (`--bubbles-per-request`، پیش‌فرض ۶) + قفل سراسری روی درخواست‌ها → دیگر ارور timeout/503 از Google نمی‌گیرید |
| **نمایشگر خواندن** | دسکتاپ: دکمهٔ «📖 خواندن» (پنجرهٔ تمام‌صفحه با اسکرول) · وب: «👁 نمایش» (خوانندهٔ تمام‌عرض HTML) + دکمهٔ دانلود |
| **ری‌استارت خودکار سرور** | لانچرها اگر سرور وب کرش کند خودکار بالا می‌آورند + ترجیح خودکار Python 3.12 (پایدارتر) |
| **پاکسازی آشغال OCR** | کاراکترهای خرابی مثل `!nn` از متن ترجمه حذف می‌شوند |
