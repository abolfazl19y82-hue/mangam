...
# --- تنگ‌کردنِ ماسکِ متن قبل از پاکسازی (فاز ۴، اختیاری) ---
# adapted از manga-image-translator/mask_refinement. مسیرِ سبک (OpenCV-only)
# همیشه در دسترسه؛ مسیرِ DenseCRF فقط اگر pydensecrf نصب باشه (نادر روی
# Colab چون کامپایلش شکننده‌ست) به‌صورت خودکار ارتقا پیدا می‌کنه.
_HAS_MASK_REFINE_MODULE = False
_mask_refine_mod = None
try:
    import mask_refine_v2 as _mask_refine_mod
    _HAS_MASK_REFINE_MODULE = True
except ImportError as _e:
    _MASK_REFINE_ERR = str(_e)
...
class MangaTranslator:
    ...
    def __init__(
        ...
        # mask_refine_mode: "auto" (پیش‌فرض: ماسکِ متن قبل از flat-fill/inpaint
        # با روش سبکِ OpenCV یا DenseCRF تنگ‌تر می‌شه)، "legacy" (رفتار قبلی،
        # بدون تنگ‌کردن)، یا "crf" (اجبار به DenseCRF؛ خطا اگر pydensecrf نصب نبود).
        self.mask_refine_mode = mask_refine_mode
        self._mask_refine_warned = False
    ...
    def _refine_text_mask_tight(self, image: np.ndarray, raw_mask: np.ndarray) -> np.ndarray:
        """قبل از flat-fill/inpaint، ماسکِ متن رو تنگ‌تر می‌کنه (فاز ۴، اختیاری).
        هیچ‌وقت raise نمی‌کنه (به‌جز حالت اجباریِ 'crf' وقتی pydensecrf نصب نیست)
        و هر خطایی باعث می‌شه دقیقاً همون raw_mask قبلی برگرده — یعنی در
        بدترین حالت رفتار قبل از فاز ۴ حفظ می‌شه."""
        mode = (self.mask_refine_mode or "auto").lower()
        if mode == "legacy":
            return raw_mask

        if not _HAS_MASK_REFINE_MODULE:
            if not self._mask_refine_warned:
                print(f"    [i] ماژول تنگ‌کردنِ ماسک در دسترس نیست ({_MASK_REFINE_ERR})؛ ماسکِ قبلی استفاده می‌شود.")
                self._mask_refine_warned = True
            return raw_mask

        prefer_crf = mode in ("auto", "crf")
        if mode == "crf" and not _mask_refine_mod.dependencies_available()[0]:
            raise RuntimeError(
                "mask_refine_mode='crf' انتخاب شده ولی pydensecrf نصب نیست: "
                f"{_mask_refine_mod.dependencies_available()[1]}\nدستور نصب: pip install pydensecrf"
            )

        try:
            refined = _mask_refine_mod.refine_text_mask(image, raw_mask, prefer_crf=prefer_crf)
            if refined is None or refined.shape != raw_mask.shape:
                return raw_mask
            return refined
        except Exception as e:
            if mode == "crf":
                raise
            if not self._mask_refine_warned:
                print(f"    [!] تنگ‌کردنِ ماسک با خطا مواجه شد ({e})؛ ماسکِ قبلی استفاده می‌شود.")
                self._mask_refine_warned = True
            return raw_mask
    ...
    def _flat_fill_uniform_components(
        ...
        mask: np.ndarray,
        ring_px: int = 6,
        min_ring_pixels: int = 40,
        std_thresh: float = 12.0,
        feather_px: int = 2,
    ):
        """اگه پوستِ (زمینه‌ی) دور و برش تقریباً یک‌رنگ باشه (مثل اکثر بالن‌های
        مانگا که سفید یا تک‌رنگ‌ان)، مستقیم با همون رنگِ زمینه پرش می‌کنه —
        نتیجه‌ی خیلی تمیزتر از inpaint. اگه زمینه بافت‌دار/رنگارنگ بود
        (پس‌زمینه‌ی پیچیده)، اون بخش رو دست‌نخورده تو ماسکِ باقی‌مونده
        نگه می‌داره تا با LaMa/OpenCV inpaint پاک بشه.

        خروجی: (تصویر با نواحیِ یک‌رنگ پرشده، ماسکِ باقی‌مونده برای inpaint)
        """
        h_img, w_img = image.shape[:2]
        filled = image.copy()
        ...
        for label in range(1, num_labels):
            ...
            local_full_mask = mask[y0:y1, x0:x1]
            local_img = image[y0:y1, x0:x1]

            ring = cv2.dilate(local_comp, ring_kernel, iterations=1)
            ring_only = cv2.bitwise_and(ring, cv2.bitwise_not(local_full_mask))
            ring_only = cv2.bitwise_and(ring_only, cv2.bitwise_not(local_comp))

            ys, xs = np.where(ring_only > 0)
            if len(ys) < min_ring_pixels:
                remaining[y0:y1, x0:x1] = cv2.bitwise_or(remaining[y0:y1, x0:x1], local_comp)
                continue

            ring_pixels = local_img[ys, xs].astype(np.float32)
            std_per_channel = np.std(ring_pixels, axis=0)
            if float(np.max(std_per_channel)) > std_thresh:
                remaining[y0:y1, x0:x1] = cv2.bitwise_or(remaining[y0:y1, x0:x1], local_comp)
                continue

            mean_color = np.median(ring_pixels, axis=0)

            comp_slice = filled[y0:y1, x0:x1]
            fill_area = local_comp > 0
            comp_slice[fill_area] = mean_color.astype(np.uint8)

            if feather_px > 0:
                alpha = cv2.GaussianBlur(
                    local_comp.astype(np.float32) / 255.0,
                    (feather_px * 2 + 1, feather_px * 2 + 1), 0
                )[..., None]
                fill_color_img = np.full_like(comp_slice, mean_color, dtype=np.float32)
                blended = (
                    comp_slice.astype(np.float32) * (1 - alpha) + fill_color_img * alpha
                )
                filled[y0:y1, x0:x1] = blended.astype(np.uint8)
    ...
    def clean_image(self, image: np.ndarray, regions: List[TextRegion]) -> np.ndarray:
        ...
        # فاز ۱: پر کردن مستقیمِ بالن‌های تخت/یک‌رنگ (بدون inpaint، نتیجه تمیزتره)
        flat_filled, remaining_mask = self._flat_fill_uniform_components(image, mask)
        flat_count = int(np.count_nonzero(mask)) - int(np.count_nonzero(remaining_mask))
        if flat_count > 0:
            print(f"  - {flat_count} پیکسل با پرکردنِ رنگِ یک‌دست پاک شد (بدون inpaint).")

        if not np.any(remaining_mask):
            print("  - پاکسازی کامل با پرکردن رنگ یک‌دست انجام شد؛ نیازی به inpaint نبود.")
            return flat_filled

        # فاز ۲: فقط برای نواحیِ باقی‌مونده (زمینه‌ی بافت‌دار/غیریک‌رنگ) از inpaint استفاده کن
        if self.use_lama:
            lama = self._get_lama()
            if lama is not None:
                try:
                    rgb = cv2.cvtColor(flat_filled, cv2.COLOR_BGR2RGB)
                    result_pil = lama(rgb, remaining_mask)
                    result_bgr = cv2.cvtColor(np.array(result_pil), cv2.COLOR_RGB2BGR)
                    print("  - پاکسازی نواحی باقی‌مونده با LaMa انجام شد.")
                    return result_bgr
                except Exception as e:
                    print(f"  [!] LaMa خطا داد ({e})؛ به OpenCV برمی‌گردیم.")

        cleaned = flat_filled.copy()

        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        dilated = cv2.dilate(remaining_mask, kernel, iterations=2)
        ...
        print("  - پاکسازی نواحی باقی‌مونده با OpenCV inpaint (دو پاس) انجام شد.")
        return cleaned
